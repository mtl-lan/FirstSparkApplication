...
object d_exe2_averageRating {

  //Improve the popularMovies exercise to sort popular movies by their average rating
  //instead of ratingcount.
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("Files/ml-100k/u.data").map(x => Movie(x.split("\t")(1).toInt, x.split("\t")(2).toInt))
